...
# Window length in seconds — 1 s gives enough packets to detect floods
WINDOW_LENGTH = 0.5
...
def extract_features(window, now):
    ...
    # Fluctuation: std-dev of inter-packet arrival times
    fluctuation = 0.0
...
# ──────────────────────────────────────────────
# Threat labelling  (highest severity wins)
# ──────────────────────────────────────────────
...
def packet_processing(pkt):
    ...
    # ── ARP layer ─────────────────────────────
    # Note: pkt[ARP] uses scapy's ARP — the manual RawARP class below
    # is kept separate and never imported under the name ARP to avoid shadowing.
...
FEATURE_NAMES = [
    ...
    "in_out_ratio",
    "fluctuation",
    "syn_only_count",      # SYN flood indicator
    "http_count",          # HTTP flood indicator
    "label",
]
...
# ──────────────────────────────────────────────
# Raw-packet helper classes  (manual parsing)
# These are kept for low-level inspection if needed.
# They do NOT shadow scapy's TCP/UDP/ARP/IP names.
# ──────────────────────────────────────────────
...
class RawUDP:
    """Manual UDP header parser (renamed to avoid shadowing scapy's UDP)."""
    def __init__(self, data):
...
class RawARP:
    """Manual ARP header parser (renamed to avoid shadowing scapy's ARP)."""
    def __init__(self, data):
